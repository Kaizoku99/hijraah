 ...
 */
export function sentryErrorLoggerMiddleware(options: { component?: string } = {}) {
  return async (c: Context, next: Next) => {
    try {
      // Start a transaction for performance monitoring
      const transaction = SentryLib.startTransaction(
        `${c.req.method} ${c.req.path}`,
        'http.server'
      );
      
      // Set transaction on scope for error tracking
      if (transaction) {
        SentryLib.setContext('transaction', { id: transaction.traceId });
      }
      ...
      SentryLib.setContext('request', SentryLib.getRequestData(c));
      ...
      const responseBody = {
        success: false,
        error: IS_PRODUCTION ? 'Internal Server Error' : err.message || 'Unknown error',
...
export function logError(errorLog: ErrorLogData): void {
  if (IS_PRODUCTION) {
    ...
    if (errorLog.component) {
      SentryLib.setContext('component', { name: errorLog.component });
    }
    
    if (errorLog.tags) {
      Object.entries(errorLog.tags).forEach(([key, value]) => {
        SentryLib.setContext(key, { value });
      });
    }
    
    // Send to Sentry
    SentryLib.captureException(error, sentryContext);
