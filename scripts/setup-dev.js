...
console.log(
  `${colors.bold}${colors.blue}=== Hijraah Development Setup ====${colors.reset}\n`,
);
...
if (missingRequiredVars.length > 0) {
  ...
  if (missingOptionalVars.length > 0) {
    console.log(
      `${colors.yellow}⚠️ Missing optional environment variables:${colors.reset}`,
    );
    ...
    console.log(
      `${colors.green}✅ All optional environment variables are set.${colors.reset}`,
    );
  }
}
...
if (!missingRequiredVars.length) {
  ...
  try {
    ...
    if (error) {
      if (error.code === "42P01") {
        console.log(
          `${colors.green}✅ Connected to Supabase successfully (Table not found, but connection works)${colors.reset}`,
        );
      } else {
        console.log(
          `${colors.yellow}⚠️ Connected to Supabase but got an error: ${error.message}${colors.reset}`,
        );
      }
    } else {
      console.log(
        `${colors.green}✅ Connected to Supabase successfully${colors.reset}`,
      );
    }
  } catch (err) {
    console.log(
      `${colors.red}❌ Failed to connect to Supabase: ${err.message}${colors.reset}`,
    );
...
try {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    ...
    if (!fs.existsSync(nodeModulesPath)) {
      console.log(
        `${colors.yellow}⚠️ node_modules directory not found. Running npm install...${colors.reset}`,
      );
      execSync("npm install", { cwd: rootDir, stdio: "inherit" });
    } else {
      console.log(
        `${colors.green}✅ node_modules directory found.${colors.reset}`,
      );
    }
    ...
    try {
      ...
      console.log(
        `${colors.yellow}⚠️ Tailwind CSS not found in node_modules. It might need to be installed.${colors.reset}`,
      );
    }
    ...
    try {
      ...
      console.log(
        `${colors.yellow}⚠️ Hono not found in node_modules. It might need to be installed.${colors.reset}`,
      );
    }
  }
} catch (error) {
  console.log(
    `${colors.red}❌ Error checking dependencies: ${error.message}${colors.reset}`,
  );
}
