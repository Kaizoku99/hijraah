...
const oldImportPatterns = [
  {
    ...
    replacement:
      "import { useAuthCompat as useAuth } from '@/hooks/auth-migration'",
  },
  {
    pattern: /import .*from ['"]@\/lib\/auth['"]/g,
    replacement:
      "import { getAuthUser, requirePermission, signOut } from '@/lib/auth-config'",
  },
  {
    pattern: /import .*from ['"]@\/components\/session-provider['"]/g,
    replacement:
      "import { useSessionCompat as useSession } from '@/hooks/auth-migration'",
  },
];
...
function main() {
  ...
  if (updatedCount > 0) {
    ...
    console.log(
      `   Some manual adjustments may be needed for complex import patterns.`,
    );
  }
}
