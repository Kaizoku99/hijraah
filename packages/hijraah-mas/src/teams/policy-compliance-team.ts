...
export class PolicyComplianceTeam {
  ...
  async checkCompliance(
    ...
    const checkCompliance = withAgentErrorHandling(async () => {
      ...
      const policyAnalysis = await this.analyzePolicies(application, currentPolicies)
      ...
      const complianceResult = await this.performComplianceCheck(
        application, 
        currentPolicies.filter(p => policyAnalysis.applicablePolicies.includes(p.id))
      )
  ...
  private async performComplianceCheck(
    ...
    const { object: complianceCheck } = await generateObject({
      ...
      maxSteps: 8,
      stopWhen: (step) => step.toolCalls.some(call => call.name === 'finalComplianceDecision'),
  ...
  async checkSpecificRequirements(
    ...
    const checkRequirements = withAgentErrorHandling(async () => {
      const { object: requirementCheck } = await generateObject({
        ...
        maxSteps: 5,
        system: 'You are a requirements verification specialist. Check applications against specific requirements.',
  ...
  async validateEligibility(
    ...
    const validateEligibility = withAgentErrorHandling(async () => {
      const { object: eligibilityCheck } = await generateObject({
        ...
        maxSteps: 5,
        system: 'You are an immigration eligibility specialist. Determine if applicants meet eligibility criteria.',
  ...
  async getPolicyUpdates(
    ...
    const getPolicyUpdates = withAgentErrorHandling(async () => {
      const { object: updates } = await generateObject({
        ...
        maxSteps: 3,
        system: 'You are a policy monitoring specialist. Track policy changes and their impacts.',
    ...
    const generateReport = withAgentErrorHandling(async () => {
      const { object: report } = await generateObject({
        ...
        schema: z.object({
          ...
          complianceStatus: z.enum(['compliant', 'non_compliant', 'requires_review']),
        ...
        system: 'You are a compliance reporting specialist. Generate comprehensive compliance reports.',
