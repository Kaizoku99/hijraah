...
export const generateSheetTool = {
  description:
    "Generates a data sheet (table) based on a query. Returns structured data.",
  parameters: z.object({
    query: z
      .string()
      .describe("The query describing the data needed for the sheet."),
    title: z
      .string()
      .optional()
      .describe("Optional title for the sheet artifact."),
  }),
  ...
  execute: async ({ query, title }: { query: string; title?: string }) => {
    ...
    const simulatedData = {
      ...
      rows: [
        ...
        {
          Name: "Alex Green",
          Specialty: "Citizenship Applications",
          Contact: "alex.green@example.com",
        },
      ],
    };
