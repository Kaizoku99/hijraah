...
export class LanguineService {
  ...
  async init(): Promise<void> {
    ...
    this.config.projectId = process.env.LANGUINE_PROJECT_ID || this.config.projectId;
    
    // Validate configuration
    if (this.config.syncMode === 'auto' && (!this.config.apiKey || !this.config.projectId)) {
      console.warn('Languine auto-sync enabled but API key or project ID missing');
    ...
    try {
      ...
      const defaultConfig = {
        ...
        glossary: {
          enabled: true,
          terms: [
            { term: 'Hijraah', translation: { ar: 'هجرة', fr: 'Hijraah', es: 'Hijraah' }, notes: 'Brand name' }
          ]
        }
      };
  ...
  async syncTranslations(force: boolean = false): Promise<boolean> {
    ...
    try {
      ...
      console.log(`Synced translations for ${this.config.targetLocales.join(', ')}`);
      return true;
    } catch (error) {
