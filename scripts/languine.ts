  ...
  .action(async () => {
    ...
    try {
      ...
      for (const locale in stats) {
        ...
        console.log(
          `  File size: ${(stats[locale].size / 1024).toFixed(2)} KB`,
        );
        console.log(
          `  Last modified: ${new Date(stats[locale].lastModified).toLocaleString()}`,
        );
      }
      ...
      if (stats["en"]) {
        ...
        for (const locale in stats) {
          ...
          const percentage = (
            (stats[locale].totalKeys / primaryCount) *
            100
          ).toFixed(1);
          console.log(
            `  ${locale}: ${percentage}% (${stats[locale].totalKeys}/${primaryCount} keys)`,
          );
        }
      }
    } catch (error) {
