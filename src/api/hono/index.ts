...
import { initializeAutoScaling, getScalingState, shutdownAutoScaling } from './lib/auto-scaling';
...
import { redisCacheMiddleware, getRedisStats, clearRedisCache, invalidateRedisCache } from './middleware/redis-cache';
import { rateLimitMiddleware, endpointRateLimiter, RATE_LIMITS, ipRateLimiter, getRateLimitStatus } from './middleware/rate-limit';
import { sentryErrorLoggerMiddleware, errorDebugHandler, initializeErrorLogging } from './middleware/sentry-error-logger';
...
app.get('/', async (c) => {
  ...
  return c.json({ 
    ...
    services: {
      ...
      scaling: process.env.AUTO_SCALING_ENABLED === 'true' ? 'enabled' : 'disabled',
    },
    docs: '/api-docs'
  });
});
...
app.get('/api-docs', redisCacheMiddleware({ ttl: 3600 }), (c) => c.json({
...
const vectorSearchLimiter = rateLimitMiddleware({
  ...
  errorMessage: 'Vector search rate limit exceeded. Please try again in a minute.'
});

const researchLimiter = rateLimitMiddleware({
  ...RATE_LIMITS.RESEARCH,
  keyGenerator: endpointRateLimiter('research'),
  errorMessage: 'Research session rate limit exceeded. Please try again in a minute.'
});

// Register route handlers with specific middleware
const researchApp = new Hono();
researchApp.use('*', researchLimiter);
researchApp.use('*', redisCacheMiddleware({ ttl: 60 })); // Short cache for research endpoints
...
scrapingApp.use('*', redisCacheMiddleware({ ttl: 300 }));
