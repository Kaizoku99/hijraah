...
export function setupImmigrationRoutes(app: Hono) {
  ...
  const compareQuerySchema = z.object({
    ...
    category: z.enum(['visa', 'citizenship', 'residence', 'business', 'tax', 'education', 'healthcare']).optional(),
  });
  ...
  app.get(
    ...
    async (c) => {
      ...
      try {
        ...
        throw new HTTPException(500, { message: 'Failed to fetch immigration data' });
      }
    }
  );
  ...
  app.post(
    ...
    async (c) => {
      ...
      try {
        ...
        throw new HTTPException(500, { message: 'Failed to compare immigration data' });
      }
    }
  );
