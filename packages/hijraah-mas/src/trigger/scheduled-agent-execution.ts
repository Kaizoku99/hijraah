  ...
  run: async (payload, { ctx, logger }) => {
    ...
    const agentRequest: AgentTaskRequest = {
      ...
      input: {
        ...
        lastCheck: payload.lastCheck || new Date(Date.now() - 3600000).toISOString() // 1 hour ago
      },
      context: {
        priority: 'high',
    ...
    try {
      ...
      logger.info('Scheduled policy monitoring completed', {
        ...
        changesDetected: result.result?.monitoringResults?.reduce(
          (sum: number, r: any) => sum + (r.changes?.length || 0), 0
        ) || 0
      })
  ...
  run: async (payload, { ctx, logger }) => {
    ...
    try {
      ...
      logger.info('Scheduled trend analysis completed', {
        success: result.success,
        trendsIdentified: result.result?.trendAnalysis?.identifiedTrends?.length || 0
      })
  ...
  run: async (payload, { ctx, logger }) => {
    ...
    try {
      ...
      logger.info('Scheduled cross-jurisdiction analysis completed', {
        success: result.success,
        jurisdictionsAnalyzed: result.result?.crossJurisdictionAnalysis?.jurisdictionComparison?.length || 0
      })
...
export const scheduledAgentExecutionTask = schedules.task({
  ...
  run: async (payload: ScheduledAgentExecution, { ctx, logger }) => {
...
async function executeScheduledAgentWithConstraints(
  ...
  const stopCondition = payload.constraints?.stopWhen 
    ? new Function('stepNumber', 'steps', 'result', `return ${payload.constraints.stopWhen}`)
    : null

  logger.info('Executing scheduled agent with constraints', {
    maxSteps: payload.constraints?.maxSteps,
  ...
  const result = await generateObject({
    ...
    maxSteps: payload.constraints?.maxSteps || 10,
    onStepFinish: ({ stepNumber: currentStep, text, toolCalls, usage, finishReason }) => {
      stepNumber = currentStep
      const stepResult = { text, toolCalls, finishReason }
      
      steps.push({
        stepNumber: currentStep,
        action: `scheduled_${payload.agentType}_step`,
        result: stepResult,
        timestamp: new Date(),
        tokenUsage: usage
      })

      logger.info(`Scheduled agent step ${currentStep} completed`, {
        finishReason,
        tokenUsage: usage
      })
      ...
      if (stopCondition && !shouldStop) {
        try {
          shouldStop = stopCondition(currentStep, steps, stepResult)
          if (shouldStop) {
            logger.info('Custom stop condition met', { step: currentStep })
          }
        } catch (error) {
          logger.warn('Error evaluating stop condition', {
            error: error instanceof Error ? error.message : String(error)
          })
        }
      }
  ...
  return {
    ...
    execution: {
      startTime: new Date(Date.now() - (result.object.metrics.processingTime || 0)),
      endTime: new Date(),
