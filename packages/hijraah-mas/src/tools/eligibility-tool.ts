...
export const eligibilityTool = createImmigrationTool(
  ...
  async ({
    ...
    const mockEligibilityCheck = {
      ...
      criteriaResults: criteria.map((criterion: any) => ({
        criterion,
        ...
        notes:
          Math.random() > 0.7
            ? `Additional verification needed for ${criterion}`
            : undefined,
    ...
    const metCriteria = mockEligibilityCheck.criteriaResults.filter(
      (c: any) => c.met
    ).length;
    ...
    mockEligibilityCheck.criteriaResults.forEach((result: any) => {
      if (result.score < 70) {
