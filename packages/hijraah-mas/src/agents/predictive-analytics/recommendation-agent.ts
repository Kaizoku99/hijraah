...
export class RecommendationAgent {
  ...
  async generateRecommendations(
    ...
    const strategicAnalysisTool = tool({
      description:
        "Analyze overall strategy based on user profile and analytics",
      parameters: z.object({
        userProfile: z.any(),
      ...
      execute: async ({
        ...
        if (successProbability && successProbability > 0.8) {
          strategies.push(
            "Direct Application Strategy - High confidence approach with standard timeline"
          );
        } else if (successProbability && successProbability > 0.6) {
          strategies.push(
            "Enhanced Preparation Strategy - Strengthen profile before application"
          );
        } else {
          strategies.push(
            "Comprehensive Improvement Strategy - Significant profile enhancement required"
          );
        }
        ...
        if (riskLevel === "high" || riskLevel === "critical") {
          strategies.push(
            "Risk Mitigation Priority - Address critical risks before proceeding"
          );
        }
        ...
        if (urgency === "critical" || urgency === "high") {
          strategies.push(
            "Expedited Processing Strategy - Fast-track where possible"
          );
        }
      ...
      execute: async ({
        ...
        if (userProfile.documents?.documentsReady < 0.8) {
          actions.push({
            ...
            description:
              "Gather all required documents and obtain certified copies",
        ...
        if (
          ...
          actions.push({
            ...
            description:
              "Accumulate required funds to demonstrate financial stability",
            category: "preparation",
            priority: "high",
        ...
        actions.push({
          ...
          description:
            "Submit comprehensive application with all supporting documents",
          category: "submission",
          priority: "critical",
          urgency: "flexible",
        ...
        actions.push({
          ...
          description:
            "Regularly check application status and respond to requests",
          category: "follow_up",
          priority: "medium",
          urgency: "flexible",
      ...
      execute: async ({ actionPlan, estimatedProcessingTime, urgency }) => {
        ...
        phases.push({
          ...
          actions: actionPlan
            .filter(
              (a: any) =>
                a.category === "preparation" || a.category === "documentation"
            )
            .map((a: any) => a.id),
          milestones: [
        ...
        phases.push({
          ...
          actions: actionPlan
            .filter((a: any) => a.category === "submission")
            .map((a: any) => a.id),
          milestones: [
        ...
        phases.push({
          ...
          duration: estimatedProcessingTime
            ? `${Math.round(estimatedProcessingTime / 30)} months`
            : "3-12 months",
          objectives: [
          ...
          actions: actionPlan
            .filter((a: any) => a.category === "follow_up")
            .map((a: any) => a.id),
          milestones: [
    ...
    const personalizationTool = tool({
      ...
      execute: async ({ userProfile, userPreferences, caseData }) => {
        ...
        if (userPreferences?.riskTolerance === "conservative") {
          customizations.push(
            "Prioritize low-risk strategies with higher certainty"
          );
          customizations.push(
            "Recommend additional documentation and preparation time"
          );
        ...
        if (userPreferences?.budgetConstraints) {
          customizations.push(
            `Optimize for budget limit of ${userPreferences.budgetConstraints}`
          );
        ...
        if (userPreferences?.timeConstraints) {
          customizations.push(
            `Optimize for timeline: ${userPreferences.timeConstraints}`
          );
        }
