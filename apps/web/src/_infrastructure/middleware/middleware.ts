...
// Get fetch from ponyfill to avoid undici issues
  ...
  try {
    ...
    if (missingVars.length > 0) {
      ...
      if (
        ...
        AUTH_REQUIRED_ROUTES.some((route) => requestPath.startsWith(route))
      ) {
    ...
    // Handle potential errors during session retrieval
    if (error || !user) {
      // No authenticated user
      return response;
    }

    // Get user metadata for headers
    ...
    // Get response
    const cacheResponse = await ponyfetch(cacheKey.toString(), {
      headers: request.headers,
    });
