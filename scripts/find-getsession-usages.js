...
const EXCLUDED_FILES = [".git", ".env", "package.json", "package-lock.json"];
...
const PATTERNS = [
  {
    ...
    description:
      "Using getSession() directly without authentication verification",
  },
  {
    regex:
      /const\s+{\s*data\s*:\s*{\s*session\s*}\s*,?\s*(?:error\s*:?\s*[a-zA-Z]*\s*)?}\s*=\s*await\s+supabase\.auth\.getSession\(\)/g,
    suggestion:
  ...
  },
  {
    ...
    suggestion: "user",
    description:
      "Using session.user directly (consider using user object from getUser() instead)",
  },
];
...
async function main() {
  ...
  if (results.length > 0) {
    ...
    console.log(
      "2. First verify the user with getUser(), then only use getSession() if you specifically need the session token",
    );
    console.log(
      "3. Avoid accessing user properties directly from session.user",
    );
  }
}
