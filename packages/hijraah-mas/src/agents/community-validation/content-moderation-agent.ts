...
export class ContentModerationAgent {
  ...
  async moderateContent(
    ...
    const spamDetectionTool = tool({
      ...
      execute: async ({ content, userHistory, similarContent }) => {
        const flags = []
        ...
        const maxWordFreq = Math.max(...Object.values(wordFreq))
        ...
        const promotionalKeywords = ['buy', 'sale', 'discount', 'offer', 'deal', 'cheap', 'free']
        ...
        if (similarContent && similarContent.length > 0) {
          const highSimilarity = similarContent.filter((s: any) => s.similarity > 0.9)
    ...
    const inappropriateContentTool = tool({
      ...
      execute: async ({ content, sensitivity }) => {
        const flags = []
        const contentText = typeof content.content === 'string' ? 
          content.content : JSON.stringify(content.content)

        // Check for inappropriate language
        const inappropriateWords = ['hate', 'discrimination', 'harassment', 'abuse']
        const inappropriateCount = inappropriateWords.filter(word => 
          contentText.toLowerCase().includes(word)
        ).length

        if (inappropriateCount > 0) {
          const severityLevel = inappropriateCount >= 3 ? 'high' : 
                               inappropriateCount >= 2 ? 'medium' : 'low'
        ...
        const misinformationIndicators = [
          'guaranteed', 'secret', 'they don\'t want you to know',
          'doctors hate this', 'miracle cure', 'instant approval'
        ]
    ...
    const qualityAssessmentTool = tool({
      ...
      execute: async ({ content, minimumQualityThreshold }) => {
        const flags = []
        const contentText = typeof content.content === 'string' ? 
          content.content : JSON.stringify(content.content)
        ...
        if (content.contentType === 'policy_update' || content.contentType === 'advice') {
        ...
        const immigrationKeywords = [
          'visa', 'immigration', 'passport', 'citizenship', 'permit', 'application',
          'embassy', 'consulate', 'border', 'refugee', 'asylum'
        ]
    ...
    const userBehaviorAnalysisTool = tool({
      ...
      execute: async ({ userHistory, communityReports }) => {
        const flags = []

        // Analyze user history for patterns
        if (userHistory && userHistory.length > 0) {
          const recentViolations = userHistory.filter((h: any) => 
            h.violations > 0 && 
        ...
        if (communityReports && communityReports.length > 0) {
          const highSeverityReports = communityReports.filter((r: any) => r.severity === 'high')
    ...
    const { object: decision } = await generateObject({
      ...
      tools: {
        detectSpam: spamDetectionTool,
        detectInappropriate: inappropriateContentTool,
        assessQuality: qualityAssessmentTool,
        analyzeBehavior: userBehaviorAnalysisTool
      },
  ...
  async generateModerationInsights(
    ...
    const { object: insights } = await generateObject({
      ...
      schema: z.object({
        ...
        trends: z.object({
          flagTypeDistribution: z.record(z.number()),
          severityTrends: z.record(z.number()),
