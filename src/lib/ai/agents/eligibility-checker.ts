...
${JSON.stringify(this.metadata.userProfile, null, 2)}`;
      ...
      const responseContent = typeof modelResponse === 'string' 
        ? modelResponse 
  ...
  private async extractUserProfileInfo(message: string): Promise<void> {
    ...
    const originMatch = message.match(/(\b[Ii]\s*am\s*from|\b[Mm]y\s*country\s*is)\s*([A-Za-z\s]+)/);
    if (originMatch && originMatch[2]) {
      this.metadata.userProfile.countryOfOrigin = originMatch[2].trim();
    }
    
    // Extract education level
    const educationMatch = message.match(/(\beducation|\bdegree|\bqualification).*?([A-Za-z\s']+)/i);
    if (educationMatch && educationMatch[2]) {
      this.metadata.userProfile.educationLevel = educationMatch[2].trim();
    }
    
    // Extract work experience
    const experienceMatch = message.match(/(\bexperience|\bworked).*?(\d+)\s*(years|year)/i);
  ...
  private async checkEligibility(): Promise<EligibilityResult[]> {
    ...
    if (profile.age && profile.workExperience) {
      const skilledWorkerResult: EligibilityResult = {
        ...
        eligible: (profile.age >= 21 && profile.age <= 45 && profile.workExperience >= 2),
        confidence: 80,
        criteria: [
          {
            id: 'age',
        ...
        explanation: `Based on your profile, you ${(profile.age >= 21 && profile.age <= 45 && profile.workExperience >= 2) ? 'are' : 'are not'} eligible for the Skilled Worker Program.`,
      };
    ...
    if (profile.age) {
      const studentVisaResult: EligibilityResult = {
        ...
        explanation: `Based on your profile, you ${profile.age >= 18 ? 'are' : 'are not'} eligible for the Student Visa Program.`,
      };
  ...
  private formatEligibilityResults(): string {
    ...
    let message = 'Based on the information you\'ve provided, here are your eligibility results:\n\n';
    ...
    if (eligiblePrograms.length > 0) {
      ...
      program.criteria.forEach(criterion => {
        message += `- ${criterion.name}: ${criterion.status === 'met' ? '✓' : '✗'} ${criterion.description} (${criterion.details})\n`;
      });
      message += '\n';
  ...
  private generateRecommendations(): string {
    ...
    if (eligiblePrograms.length > 0) {
      ...
      recommendations += '4. Plan for medical examinations and background checks\n\n';
    }
    ...
    if (ineligiblePrograms.length > 0) {
      ...
      if (unmetCriteria.has('age')) {
        recommendations += '- Consider programs without strict age requirements\n';
      }
      ...
      if (unmetCriteria.has('language')) {
        recommendations += '- Improve language proficiency and take official tests\n';
      }
    }
   ...
   */
  private async extractProgramAndCriteriaMentions(content: string): Promise<void> {
