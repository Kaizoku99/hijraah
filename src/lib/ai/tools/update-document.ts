 ...
 */
export function updateDocumentTool({ session, dataStream }: { 
  ...
  return {
    ...
    parameters: z.object({
      ...
      content: z.string().describe('New content or changes to make to the document'),
      appendOnly: z.boolean().optional().default(false).describe('If true, append content instead of replacing it'),
    }),
    execute: async ({ 
      id,
      title, 
