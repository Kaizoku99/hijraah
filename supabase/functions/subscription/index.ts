...
async function getSubscription(supabaseClient, userId: string) {
...
async function createCheckout(supabaseClient, user: User, planId: string) {
...
async function cancelSubscription(supabaseClient, userId: string) {
  const { data, error } = await supabaseClient
    .from('subscriptions')
    .update({
      status: 'canceled',
...
async function handleWebhook(supabaseClient, payload: WebhookPayload) {
