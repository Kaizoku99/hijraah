// Add Deno TypeScript declarations for IDE support
 ...
 */
async function generateCacheKey(content: string, query: string): Promise<string> {
...
function checkRateLimit(ip: string): boolean {
  ...
  while (
    ...
    rateLimitBuckets[0].resetAt < now
  ) {
...
async function checkCache(cacheKey: string): Promise<AnalyzeResponse | null> {
  try {
    ...
    console.error(`Cache lookup error: ${error instanceof Error ? error.message : String(error)}`);
    metrics.cacheMisses++;
    return null;
  }
}
...
async function saveToCache(
  ...
  try {
    ...
    console.error(`Cache save error: ${error instanceof Error ? error.message : String(error)}`);
  }
}
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          console.error(`[${requestId}] Failed to extract JSON from response:`, extractError);
          analysis = { 
 ...
 */
async function handleMetricsRequest(req: Request, requestId: string): Promise<Response> {
...
Deno.serve(async (req: Request) => {
  ...
  console.info(`[${requestId}] Request received: ${req.method} ${new URL(req.url).pathname}`);
  ...
  try {
    ...
    const { content, query, depth = 1, skipCache = false, cacheKey: providedCacheKey } = requestBody;

    // Generate a cache key if not provided
    const cacheKey = providedCacheKey || await generateCacheKey(content, query);
    ...
    console.info(`[${requestId}] Analyzing content with query: "${query.substring(0, 50)}..." (depth: ${depth})`);

    // Process analysis request using OpenAI
    const analysisResult = await analyzeContent(content, query, depth, requestId);
    ...
    if (analysisResult.success && !skipCache) {
      ...
      if (typeof EdgeRuntime !== 'undefined') {
        EdgeRuntime.waitUntil(cachePromise.catch(err => {
