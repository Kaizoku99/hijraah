...
export const CreateArtifactTool = createTool({
  ...
  description:
    "Create a new artifact (document, code, spreadsheet, etc.) from the conversation",
  schema: z.object({
    title: z.string().describe("The title of the artifact"),
    type: z
      .enum(["document", "code", "spreadsheet", "image", "mindmap"] as const)
      .describe("The type of artifact to create"),
    content: z
      .any()
      .describe(
        "The content of the artifact. For documents, include a text field. For code, include code, language, and fileName fields.",
      ),
    visibility: z
      .enum(["private", "public", "team"] as const)
      .optional()
      .default("private")
      .describe("The visibility of the artifact"),
    chat_id: z
      .string()
      .optional()
      .describe("The ID of the chat this artifact is associated with"),
  }),
  execute: async (
    { title, type, content, visibility, chat_id },
    { session },
  ) => {
    try {
      if (!session?.user?.id) {
...
export const UpdateArtifactTool = createTool({
  ...
  schema: z.object({
    ...
    visibility: z
      .enum(["private", "public", "team"] as const)
      .optional()
      .describe("The new visibility setting for the artifact"),
  }),
