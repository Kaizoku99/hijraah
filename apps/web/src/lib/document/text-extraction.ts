...
import { processMistralOCR, processMistralOCRWithFileUpload, extractPlainTextFromOCRResponse } from '@/lib/ai/mistral-ocr';
 ...
 */
export async function extractTextFromImage(imageUrl: string): Promise<{ text: string; buffer: Buffer }> {
  try {
    // Generate a presigned URL if it's a Supabase URL
 ...
 */
export async function extractTextFromPdfWithMistralOCR(documentUrl: string): Promise<string> {
  try {
    ...
    throw new Error(`Failed to extract text from PDF with Mistral OCR: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
 ...
 */
export async function extractTextFromImageWithMistralOCR(imageUrl: string): Promise<{ text: string; buffer: Buffer }> {
  try {
    // Generate a presigned URL if it's a Supabase URL
    let accessibleUrl = imageUrl;
    if (imageUrl.includes('supabase')) {
    ...
    const ocrResponse = await processMistralOCRWithFileUpload(buffer, filename, {
      preserveFormatting: true,
      detectTables: true
    });
    ...
    throw new Error(`Failed to extract text from image with Mistral OCR: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
