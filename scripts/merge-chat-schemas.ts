...
async function main() {
  ...
  try {
    ...
    if (hasChats && hasAiConversations) {
      console.log(
        "Both chats and ai_conversations tables exist. Merging data...",
      );
      ...
      if (aiConversations && aiConversations.length > 0) {
        console.log(
          `Found ${aiConversations.length} AI conversations to migrate`,
        );
        ...
        for (const conversation of aiConversations) {
          ...
          if (!existingChat) {
            ...
            if (insertError) {
              console.error(
                `Error inserting chat ${conversation.id}: ${insertError.message}`,
              );
            }
          }
        }
    ...
    if (hasChatMessages && hasAiMessages) {
      console.log(
        "Both chat_messages and ai_messages tables exist. Merging data...",
      );
      ...
      if (aiMessages && aiMessages.length > 0) {
        ...
        for (const message of aiMessages) {
          ...
          if (!existingMessage) {
            ...
            if (insertError) {
              console.error(
                `Error inserting message ${message.id}: ${insertError.message}`,
              );
            }
          }
        }
      ...
      console.log(
        "Only ai_messages table exists. Renaming to chat_messages...",
      );
      // Rename ai_messages to chat_messages
      await supabase.rpc("rename_table", {
        old_name: "ai_messages",
