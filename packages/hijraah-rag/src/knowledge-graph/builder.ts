...
import { RAGProcessedDocument, KGEntity, KGRelationship } from "../types.js";
import { v4 as uuidv4 } from "uuid";

// Context7 Pattern: Type-safe entity and relationship schemas
...
export class KnowledgeGraphBuilder {
  ...
  async buildFromDocument(document: RAGProcessedDocument): Promise<{
    ...
    // Convert to KG types
    const kgEntities: KGEntity[] = entities.map((e) => ({
      id: e.id,
      name: e.name,
      type: e.type,
    ...
    const kgRelationships: KGRelationship[] = relationships.map((r) => ({
      id: r.id,
      sourceEntityId: r.sourceEntityId,
      targetEntityId: r.targetEntityId,
      sourceEntityName:
        entities.find((e) => e.id === r.sourceEntityId)?.name || "",
      targetEntityName:
        entities.find((e) => e.id === r.targetEntityId)?.name || "",
      type: r.type,
