...
export function useHijraahApi() {
  ...
  const scraping = {
    ...
    scrapeUrl: async (
      ...
      return apiRequest<{
        ...
        metadata: any;
        storageUrl?: string;
      }>("scraping/scrape", "POST", { url, selector, saveToStorage });
    },
  ...
  const admin = {
    ...
    clearCache: async () => {
      return apiRequest("/admin/cache/clear", "POST");
    },

    invalidateCache: async (pattern: string) => {
      return apiRequest("/admin/cache/invalidate", "POST", { pattern });
    },
    ...
    resetPerformanceMetrics: async () => {
      return apiRequest("/admin/performance/reset", "POST");
    },
