...
export class ContentExtractionAgent {
  ...
  async extractContent(
    ...
    const extractContent = withAgentErrorHandling(async () => {
      ...
      const dynamicExtractionSchema =
        this.createExtractionSchema(extractionSchema);
  ...
  async extractSpecializedFields(
    ...
    const extractSpecialized = withAgentErrorHandling(async () => {
      const specializedSchema =
        this.createSpecializedExtractionSchema(fieldTypes);
  ...
  validateExtractedContent(
    ...
    for (const [field, rules] of Object.entries(validationRules)) {
      const rulesTyped = rules as { required?: boolean; minConfidence?: number; pattern?: string };
      const value = (result.extractedFields as Record<string, any>)[field];
      const confidence = (result.fieldConfidence as Record<string, number>)[field];

      // Check if required field is missing
      if (rulesTyped.required && (value === null || value === undefined)) {
        errors.push(`Required field '${field}' is missing`);
        continue;
      }

      // Check confidence threshold
      if (typeof confidence === 'number' && confidence < (rulesTyped.minConfidence || 0.7)) {
        warnings.push(`Field '${field}' has low confidence: ${confidence}`);
      }

      // Apply field-specific validation
      if (rulesTyped.pattern && typeof value === 'string') {
        const regex = new RegExp(rulesTyped.pattern);
        if (!regex.test(value)) {
      ...
      if (rules.minLength && value && value.length < rules.minLength) {
        errors.push(
          `Field '${field}' is too short (minimum ${rules.minLength} characters)`
        );
      }

      if (rules.maxLength && value && value.length > rules.maxLength) {
        warnings.push(
          `Field '${field}' is very long (maximum ${rules.maxLength} characters recommended)`
        );
      }
    }
