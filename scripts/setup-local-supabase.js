...
`;
    ...
    print('IMPORTANT: Update the OPENAI_API_KEY in .env.local.dev with your actual key.', colors.yellow);
  } else {
...
async function setupLocalSupabase() {
  ...
  if (!fs.existsSync(path.join(process.cwd(), 'supabase', 'config.toml'))) {
    print('Supabase is not initialized. Running supabase init...', colors.yellow);
