...
import { ChatService, ChatMessage } from '@/lib/supabase/chat';
...
interface ChatContextType {
  ...
  updateMessage: (id: string, updateFn: (message: Message) => Message) => Promise<void>;
...
export function ChatProvider({
  ...
  const activeUsers = React.useMemo(() => {
    return Object.values(presenceState).flat().map((presence) => ({
  ...
  const addMessage = React.useCallback(async (message: Message) => {
    try {
      if (!conversationId) {
  ...
  const updateMessage = React.useCallback(async (
    id: string,
  ...
  const removeMessage = React.useCallback(async (id: string) => {
    try {
