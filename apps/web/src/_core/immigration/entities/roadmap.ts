...
export class Roadmap {
  ...
  constructor(props: {
    ...
    this.estimatedEndDate = props.estimatedEndDate
      ? new Date(props.estimatedEndDate)
      : new Date();
  ...
  getCurrentPhase(): RoadmapPhase | null {
    ...
    const notStartedPhases = this.phases
  ...
  getCurrentMilestone(): RoadmapMilestone | null {
    ...
    const notStartedMilestones = currentPhase.milestones
  ...
  updateMilestoneStatus(
    ...
    const updatedPhases = this.phases.map((phase) => {
      ...
      if (milestoneIndex !== -1) {
        ...
        const anyInProgress = updatedMilestones.some(
          (m) => m.status === "in_progress",
        );
        const anyBlocked = updatedMilestones.some(
          (m) => m.status === "blocked",
        );
        const anyOverdue = updatedMilestones.some(
          (m) => m.status === "overdue",
        );
