...
console.log(
  `${colors.bold}${colors.blue}=== Tailwind CSS Configuration Update ====${colors.reset}\n`,
);
...
try {
  console.log(
    `${colors.cyan}Checking for required Tailwind plugins...${colors.reset}`,
  );
  ...
  try {
    ...
    console.log(
      `${colors.green}✅ @tailwindcss/typography is installed.${colors.reset}`,
    );
  } catch (e) {
    console.log(
      `${colors.yellow}⚠️ @tailwindcss/typography not found. Installing...${colors.reset}`,
    );
    execSync("npm install -D @tailwindcss/typography", {
      cwd: rootDir,
      stdio: "inherit",
    });
  }
  ...
  try {
    ...
    console.log(
      `${colors.green}✅ tailwindcss-animate is installed.${colors.reset}`,
    );
  } catch (e) {
    console.log(
      `${colors.yellow}⚠️ tailwindcss-animate not found. Installing...${colors.reset}`,
    );
    execSync("npm install -D tailwindcss-animate", {
      cwd: rootDir,
      stdio: "inherit",
    });
  }
  ...
  console.log(
    `${colors.red}❌ Error checking Tailwind plugins: ${error.message}${colors.reset}`,
  );
}
...
export default {
  ...
  console.log(
    `${colors.green}✅ Created new tailwind.config.js file.${colors.reset}`,
  );
} else {
  ...
  if (!existingConfig.includes('border: "hsl(var(--border))"')) {
    console.log(
      `${colors.yellow}⚠️ tailwind.config.js is missing theme configuration for Shadcn UI.${colors.reset}`,
    );
...
export default {
  ...
  plugins: [require("@tailwindcss/typography"), require("tailwindcss-animate")],
    ...
    console.log(
      `${colors.green}✅ Updated tailwind.config.js with proper theme configuration.${colors.reset}`,
    );
  } else {
    console.log(
      `${colors.green}✅ tailwind.config.js already has proper theme configuration.${colors.reset}`,
    );
  }
}
...
if (fs.existsSync(packageJsonPath)) {
  ...
  if (!packageJson.scripts["build:css"]) {
    console.log(
      `${colors.yellow}⚠️ Adding build:css script to package.json...${colors.reset}`,
    );

    packageJson.scripts["build:css"] =
      "tailwindcss -i ./apps/web/src/app/globals.css -o ./public/styles.css";
    fs.writeFileSync(
      packageJsonPath,
    ...
    console.log(
      `${colors.green}✅ Added build:css script to package.json.${colors.reset}`,
    );
  } else {
    console.log(
      `${colors.green}✅ build:css script already exists in package.json.${colors.reset}`,
    );
  }
} else {
...
console.log(
  `2. If you encounter any CSS issues, run ${colors.cyan}node scripts/update-tailwind.js${colors.reset} again`,
);
console.log(
  `3. For a production build with optimized CSS, run ${colors.cyan}npm run build:css${colors.reset} before ${colors.cyan}npm run build${colors.reset}`,
);
