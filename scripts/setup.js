...
console.log(`${colors.bold}${colors.blue}=== Hijraah Project Setup ====${colors.reset}\n`);
...
function makeExecutable(scriptPath) {
  try {
    ...
    if (!fs.existsSync(scriptPath)) {
      console.log(`${colors.yellow}⚠️ Script ${scriptPath} not found, skipping${colors.reset}`);
      return false;
    }
    ...
    if (process.platform !== 'win32') {
      ...
      console.log(`${colors.green}✅ Made ${scriptPath} executable${colors.reset}`);
    } else {
      console.log(`${colors.yellow}⚠️ Skipping chmod on Windows for ${scriptPath}${colors.reset}`);
    }
    
    return true;
  } catch (error) {
    console.log(`${colors.red}❌ Error making ${scriptPath} executable: ${error.message}${colors.reset}`);
    return false;
  }
}
...
console.log(`${colors.green}✅ Made ${executableScriptsCount} scripts executable${colors.reset}`);
...
try {
  ...
  console.log(`${colors.red}❌ Error updating Tailwind configuration: ${error.message}${colors.reset}`);
}
...
try {
  ...
  console.log(`${colors.red}❌ Error checking Supabase configuration: ${error.message}${colors.reset}`);
}
...
try {
  ...
  console.log(`${colors.red}❌ Error setting up development environment: ${error.message}${colors.reset}`);
}
...
console.log(`2. Visit ${colors.cyan}http://localhost:3000${colors.reset} to see your application`);
console.log(`3. If you encounter issues, run ${colors.cyan}node scripts/setup.js${colors.reset} again\n`); 