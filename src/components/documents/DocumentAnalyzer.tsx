...
import { FileText, Upload, AlertTriangle, Check, X, FilePlus } from 'lucide-react';
import { DocumentType, DocumentAnalysis } from '@/types/documents';
        ...
        <Card className={className}>
            ...
            <Tabs defaultValue="upload">
                <TabsList className="mx-6">
                    ...
                    <TabsTrigger value="results" onClick={() => setActiveTab('results')} disabled={!analysisResult}>Results</TabsTrigger>
                </TabsList>
                ...
                <CardContent className="pt-6">
                    ...
                    {activeTab === 'results' && analysisResult && (
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium">
                                        Completeness: {Math.round(analysisResult.completeness * 100)}%
                                    </span>
                                </div>
                            ...
                            <div className="space-y-4">
                                <Alert variant={analysisResult.formatErrors.length > 0 ? 'destructive' : 'default'}>
                                    ...
                                    <AlertDescription>
                                        {analysisResult.formatErrors.length === 0 ? (
                                            ...
                                            `${analysisResult.formatErrors.length} format ${analysisResult.formatErrors.length === 1 ? 'issue' : 'issues'
                                            } detected`
                                        )}
                                    </AlertDescription>
                                </Alert>
                                ...
                                {analysisResult.formatErrors.length > 0 && (
                                    <Accordion type="single" collapsible className="w-full">
                                        {analysisResult.formatErrors.map((error, index) => (
                                            <AccordionItem key={index} value={`format-error-${index}`}>
                                                <AccordionTrigger className="flex items-center gap-2">
                                                    <Badge variant={error.type === 'critical' ? 'destructive' : 'outline'}>
                                                        {error.type}
                                                    </Badge>
                                                ...
                                                <AccordionContent>
                                                    ...
                                                    {error.position && (
                                                        <div className="mt-2">
                                                            <p className="text-sm text-muted-foreground">
                                                                Position: x: {error.position.x}, y: {error.position.y}
                                                            </p>
                                                        </div>
                                                    )}
                                                </AccordionContent>
                            ...
                            <div className="space-y-4">
                                <Alert variant={analysisResult.contentErrors.length > 0 ? 'destructive' : 'default'}>
                                    <AlertTriangle className="h-4 w-4" />
                                    ...
                                    <AlertDescription>
                                        {analysisResult.contentErrors.length === 0 ? (
                                            ...
                                            `${analysisResult.contentErrors.length} content ${analysisResult.contentErrors.length === 1 ? 'issue' : 'issues'
                                            } detected`
                                        )}
                                    </AlertDescription>
                                </Alert>
                                ...
                                {analysisResult.contentErrors.length > 0 && (
                                    <Accordion type="single" collapsible className="w-full">
                                        {analysisResult.contentErrors.map((error, index) => (
                                            <AccordionItem key={index} value={`content-error-${index}`}>
                                                <AccordionTrigger className="flex items-center gap-2">
                                                    <Badge variant={error.type === 'critical' ? 'destructive' : 'outline'}>
                                                        {error.type}
                                                    </Badge>
