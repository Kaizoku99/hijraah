...
export function ApplicationTimeline({
    ...
    const getStatusIcon = (status: TimelineEventStatus, customIcon?: React.ReactNode) => {
    ...
    const getEventIcon = (event: TimelineEvent) => {
        ...
        } else if (event.title.toLowerCase().includes('interview') || event.title.toLowerCase().includes('appointment')) {
            return <Users className="h-5 w-5" />;
        } else if (event.title.toLowerCase().includes('processing') || event.title.toLowerCase().includes('review')) {
                                ...
                                <div className={cn(
                                    ...
                                    <div className="flex flex-col items-center">
                                        <div className={cn(
                                            ...
                                            event.status === 'rejected' && 'border-red-500 bg-red-50',
                                            event.status === 'warning' && 'border-amber-500 bg-amber-50',
                                        ...
                                        {!isLast && (
                                            <div className={cn(
                                                ...
                                                showSubEvents && event.subEvents?.length && 'min-h-[80px]'
                                            )} />
                                        )}
                                    </div>
                                    ...
                                    <div className="flex-1 pt-1 pb-6">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <Badge
                                                variant="outline"
                                                className={cn('text-xs whitespace-nowrap', getStatusColor(event.status))}
                                            >
                                        ...
                                        <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-500">
                                            ...
                                            {showExpectedDates && event.estimatedCompletionDate && event.status !== 'completed' && (
                                                <div className="flex items-center">
                                                    <Tooltip>
                                                        <TooltipTrigger className="flex items-center cursor-help">
                                                            <CalendarClock className="h-3.5 w-3.5 mr-1.5 text-gray-400" />
                                                            <span>{t('estimatedCompletion')}: {format(event.estimatedCompletionDate, 'PPP')}</span>
                                                        </TooltipTrigger>
                                                        <TooltipContent>
                                        ...
                                        {/* Sub-events */}
                                        {showSubEvents && event.subEvents && event.subEvents.length > 0 && (
                                            <div className="mt-4 pl-3 border-l-2 border-dashed border-gray-200">
                                                <div className="space-y-3">
                                                    {event.subEvents.map((subEvent) => (
                                                        <div key={subEvent.id} className="flex items-start gap-3">
                                                            <div className="w-5 h-5 rounded-full flex items-center justify-center">
                                                                {getStatusIcon(subEvent.status, subEvent.icon)}
                                                            </div>

                                                            <div className="flex-1">
                                                                <div className="flex items-start justify-between">
                                                                    <h4 className="text-sm font-medium">{subEvent.title}</h4>

                                                                    {subEvent.status && (
                                                                        <Badge
                                                                            variant="outline"
                                                                            className={cn('text-xs px-1.5 py-0 h-5', getStatusColor(subEvent.status))}
                                                                        >
                                                                ...
                                                                {subEvent.description && (
                                                                    <p className="text-xs text-gray-500 mt-0.5">{subEvent.description}</p>
                                                                )}
