...
export function compareDocumentAnalysis(
  ...
  for (const field of allFields) {
    ...
    if (source1Value !== source2Value) {
      fieldDifferences.push({
        ...
        source1Value,
        source2Value,
        isSignificant: isSignificantDifference(field, source1Value, source2Value)
      });
    }
  }
  ...
  const source1ErrorCount = source1.formatErrors.length + source1.contentErrors.length;
  const source2ErrorCount = source2.formatErrors.length + source2.contentErrors.length;
...
function isSignificantDifference(field: string, value1: any, value2: any): boolean {
  // Date fields - check if dates are more than a day apart
  if (field.toLowerCase().includes('date') && isValidDate(value1) && isValidDate(value2)) {
  ...
  if (typeof value1 === 'string' && typeof value2 === 'string') {
    ...
    if (maxLength > 0) {
      const lengthDiff = Math.abs(value1.length - value2.length) / maxLength * 100;
      if (lengthDiff > 20) return true;
    }
  }
  
  // For critical fields, any difference is significant
  const criticalFields = ['passport_number', 'id_number', 'name', 'surname', 'nationality'];
  if (criticalFields.some(critField => field.toLowerCase().includes(critField))) {
