...
export function setupDocumentRoutes(app: Hono) {
  ...
  const documentSchema = z.object({
    ...
    type: z.enum(['passport', 'visa', 'birth_certificate', 'marriage_certificate', 'identification', 'other']),
    ...
    async (c: Context) => {
      ...
      try {
        ...
        if (fetchError || !existing) {
          throw new HTTPException(404, { message: 'Document not found or access denied' });
        }
        ...
        if (result.data.expiryDate !== undefined) updateData.expiry_date = result.data.expiryDate;
        if (result.data.countryOfIssue !== undefined) updateData.country_of_issue = result.data.countryOfIssue;
        if (result.data.description !== undefined) updateData.description = result.data.description;
        if (result.data.fileKey !== undefined) updateData.file_key = result.data.fileKey;
        if (result.data.fileType !== undefined) updateData.file_type = result.data.fileType;
        if (result.data.fileSize !== undefined) updateData.file_size = result.data.fileSize;
