...
export class DocumentProcessingTeam {
   ...
   */
  private async performQualityAssurance(processingResults: any[]) {
   ...
   */
  async extractSpecificData(documents: Document[], dataFields: string[]) {
    const extractData = withAgentErrorHandling(async () => {
      const { object: extractedData } = await generateObject({
        ...
        system:
          "You are a data extraction specialist. Extract specific fields from immigration documents accurately.",
  ...
  async checkDocumentCompliance(documents: Document[], requirements: string[]) {
    const checkCompliance = withAgentErrorHandling(async () => {
      const { object: compliance } = await generateObject({
        ...
        system:
          "You are an immigration compliance specialist. Check documents against requirements.",
