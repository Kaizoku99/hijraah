...
export class GamificationAgent {
  ...
  async updateGamificationProfile(
    ...
    const pointsCalculationTool = tool({
      ...
      execute: async ({ contributions, reviews, communityActions, currentLevel }) => {
        let totalPoints = 0
        ...
        contributions.forEach((contrib: any) => {
          ...
          const points = Math.round(basePoints * qualityMultiplier * impactMultiplier * (1 + levelBonus))
          totalPoints += points
        })
        ...
        reviews.forEach((review: any) => {
          ...
          const points = Math.round(basePoints * qualityMultiplier * helpfulnessMultiplier)
          totalPoints += points
        })
    ...
    const achievementCheckTool = tool({
      ...
      execute: async ({ contributor, currentProfile, recentActivity }) => {
        ...
        if (totalContributions >= 100 && !currentProfile.achievements.some((a: any) => a.achievementId === 'contributor_100')) {
        ...
        if (totalReviews >= 50 && !currentProfile.achievements.some((a: any) => a.achievementId === 'reviewer_50')) {
          newAchievements.push({
        ...
        if (currentProfile.streaks.currentContributionStreak >= 30 && 
            !currentProfile.achievements.some((a: any) => a.achievementId === 'streak_30')) {
          newAchievements.push({
        ...
        const highQualityContributions = recentActivity.contributions.filter((c: any) => c.quality >= 8).length
        if (highQualityContributions >= 10 && 
            !currentProfile.achievements.some((a: any) => a.achievementId === 'quality_expert')) {
          newAchievements.push({
        ...
        const helpActions = recentActivity.communityActions.filter((a: any) => a.type === 'help').length
        if (helpActions >= 25 && 
            !currentProfile.achievements.some((a: any) => a.achievementId === 'community_helper')) {
          newAchievements.push({
    ...
    const streakCalculationTool = tool({
      ...
      execute: async ({ recentActivity, currentStreaks }) => {
        ...
        const hasContributionToday = recentActivity.contributions.some((c: any) => 
          new Date(c.date).toDateString() === today.toDateString()
        )
        const hasContributionYesterday = recentActivity.contributions.some((c: any) => 
        ...
        if (hasContributionToday) {
          if (hasContributionYesterday || currentStreaks.currentContributionStreak === 0) {
            newContributionStreak = currentStreaks.currentContributionStreak + 1
          }
        ...
        // Check review streak
        const hasReviewToday = recentActivity.reviews.some((r: any) => 
          new Date(r.date).toDateString() === today.toDateString()
        )
        const hasReviewYesterday = recentActivity.reviews.some((r: any) => 
        ...
        return {
          ...
          longestContributionStreak: Math.max(currentStreaks.longestContributionStreak, newContributionStreak),
          currentReviewStreak: newReviewStreak,
          longestReviewStreak: Math.max(currentStreaks.longestReviewStreak, newReviewStreak)
        }
      }
    })
    ...
    const { object: challenges } = await generateObject({
      ...
      schema: z.object({
        challenges: z.array(z.object({
          ...
          type: z.enum(['contribution', 'review', 'streak', 'quality', 'community']),
