...
export class TranslateService {
      ...
      if (this.locale !== defaultLocale) {
        try {
          const fallbackPath = path.resolve(
            process.cwd(),
            `src/locales/${defaultLocale}.json`,
          );
  ...
  private async saveTranslationMemory(): Promise<void> {
    ...
    try {
      ...
      await fs.writeFile(
        memoryPath,
        JSON.stringify(this.translationMemory, null, 2),
      );
    } catch (error) {
  ...
  translate(key: string, variables: Record<string, any> = {}): string {
    ...
    if (this.memoryEnabled && this.translationMemory[key]?.[this.locale]) {
  ...
  getMessages(): Record<string, any> {
    if (!this.initialized) {
      console.warn(
        "TranslateService not initialized, returning empty messages",
      );
      return {};
    }
