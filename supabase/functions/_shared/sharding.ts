...
export async function getShardClient(
  ...
  try {
    ...
    const { data, error } = await centralClient.rpc(
      "sharding.get_connection_string",
      {
    ...
    if (!data) {
      console.warn(
        "No shard connection string found for",
        entityType,
        entityId,
      );
 ...
 */
function getOrCreateClientForConnectionString(
  connectionString: string,
): SupabaseClient {
...
export async function executeAcrossAllShards<T>(
  ...
  try {
    ...
    const { data, error } = await centralClient.rpc(
      "sharding.get_all_shard_connections",
      {
        p_for_write: forWrite,
      },
    );
    ...
    const results = await Promise.all(
      data.map(
        async (conn: { shard_key: string; connection_string: string }) => {
          const client = getOrCreateClientForConnectionString(
