...
export function useAnalysis() {
  ...
  const analyze = async ({ sources, model, options }: AnalysisParams) => {
    try {
      ...
      const [timelineResults, credibilityResults, crossRefResults] =
        await Promise.all([
          // Timeline analysis
          ...
          Promise.all(
            sources.map((s) =>
              new CredibilityScorer(modelManager).scoreSource(s.content, {
                domain: new URL(s.url).hostname,
                publishDate: s.metadata?.date
                  ? new Date(s.metadata.date)
                  : undefined,
