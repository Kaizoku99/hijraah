 ...
 */
export function generateReadableId(prefix?: string, length: number = 8): string {
  const timestamp = Date.now().toString(36);
  const randomChars = Math.random().toString(36).substring(2, 2 + length);
  return prefix ? `${prefix}-${timestamp}-${randomChars}` : `${timestamp}-${randomChars}`;
}
 ...
 */
export function generateSequentialId(prefix: string, counter: number, padding: number = 4): string {
