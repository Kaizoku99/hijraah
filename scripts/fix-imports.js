...
function updateFile(filePath) {
  ...
  try {
    ...
    const regex = /export \* from "@\/presentation\/components\/ui\/([^\/]+)\/([^"]+)";/;
    ...
    if (match) {
      ...
      } else if (dirName.includes('ui/ui') || dirName.includes('ui\\ui') || 
                 dirName.includes('components/ui') || dirName.includes('components\\ui')) {
        // If in the ui folder, reference the atoms folder
        newContent = content.replace(
          regex,
          `export * from "@/shared/ui/${componentType}/${componentName}/index";`
        );
      } else if ((dirName.includes('ui/molecules') || dirName.includes('ui\\molecules')) && componentType === 'molecules') {
        newContent = content.replace(
          regex,
          `export * from "./${componentName}/index";`
        );
      } else if ((dirName.includes('ui/organisms') || dirName.includes('ui\\organisms')) && componentType === 'organisms') {
        newContent = content.replace(
          regex,
          `export * from "./${componentName}/index";`
        );
      } else if ((dirName.includes('ui/templates') || dirName.includes('ui\\templates')) && componentType === 'templates') {
        newContent = content.replace(
          regex,
          `export * from "./${componentName}/index";`
        );
