...
describe("Community Validation Agents", () => {
  ...
  beforeEach(() => {
    ...
    mockContent = {
      ...
      content:
        "I recently got my Canada PR through Express Entry. Here is my detailed experience...",
      metadata: {
        country: "Canada",
  ...
  describe("PeerReviewAgent", () => {
    ...
    it("should conduct peer review with valid inputs", async () => {
      ...
      const result = await agent.conductPeerReview(
        mockContent,
        mockContributor
      );
    ...
    it("should build consensus from multiple reviews", async () => {
      ...
      const result = await agent.buildConsensus(
        "content123",
        mockReviews as any
      );
  ...
  describe("ReputationScoringAgent", () => {
    ...
    it("should calculate reputation score", async () => {
        ...
        factors: [
          {
            ...
            impact: 2.4,
            description:
              "High quality contributions with good community impact",
          },
        ],
        achievements: [],
        penalties: [],
      ...
      const result = await agent.calculateReputationScore(
        mockContributor,
  ...
  describe("ContentModerationAgent", () => {
    ...
    it("should moderate content and make decisions", async () => {
      const mockModerationDecision = {
        ...
        decision: "approve",
        reasoning:
          "Content meets community standards with minor quality concerns",
        confidence: 0.8,
  ...
  describe("GamificationAgent", () => {
    ...
    it("should update gamification profile", async () => {
      ...
      const mockRecentActivity = {
        ...
        communityActions: [{ type: "help", impact: 7, date: "2024-01-01" }],
      };
      ...
      expect(result.totalPoints).toBeGreaterThanOrEqual(
        mockCurrentProfile.totalPoints
      );
  ...
  describe("CommunityValidationTeam", () => {
    ...
    it("should perform comprehensive community validation", async () => {
      ...
      const result = await team.performCommunityValidation(
        mockContent,
        mockReviewers
      );
    ...
    it("should perform quick validation", async () => {
      ...
      const result = await team.performQuickValidation(
        mockContent,
        mockReviewers
      );
  ...
  describe("Integration Scenarios", () => {
    it("should handle high-conflict scenarios requiring consensus", async () => {
      ...
      vi.mocked(generateObject)
        .mockResolvedValueOnce({
          object: { overallScore: 9, recommendation: "approve" },
        } as any)
        .mockResolvedValueOnce({
          object: { overallScore: 3, recommendation: "reject" },
        } as any);
    ...
    it("should handle flagged content appropriately", async () => {
      ...
      const result = await team.performCommunityValidation(mockContent, [
        mockContributor,
      ]);
