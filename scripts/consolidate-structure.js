...
const getAtomicComponentName = (filePath) => {
  ...
  const uiMatches = [
    /button/i, /input/i, /card/i, /avatar/i, /badge/i, 
    /checkbox/i, /label/i, /switch/i, /textarea/i, /form/i,
    /modal/i, /dropdown/i, /alert/i, /toast/i, /tooltip/i,
    /menu/i, /nav/i, /tab/i, /dialog/i, /popover/i
  ];
...
const consolidateDirectory = async (sourceDir, targetDir) => {
  try {
    ...
    console.log(`Consolidated ${copyCount} files from ${sourceDir} to ${targetDir}`);
    return copyCount;
  } catch (error) {
...
const main = async () => {
  ...
  console.log('and manually remove the old directories once you\'ve confirmed everything works.');
  ...
  try {
    ...
    if (fs.existsSync(supabaseConfigPath)) {
      ...
      if (configContent.includes('shadow_port = 54320')) {
        configContent = configContent.replace('shadow_port = 54320', 'shadow_port = 54800');
