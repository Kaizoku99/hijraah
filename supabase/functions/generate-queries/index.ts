...
`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a research analyst that generates targeted follow-up questions to deepen research.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          queriesData = { queries: [`More specific information about ${originalQuery}`] };
        }
      } else {
        queriesData = { queries: [`More specific information about ${originalQuery}`] };
      }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        queries: queriesData.queries || [`More specific information about ${originalQuery}`],
      }),
      {
        status: 200,
        headers: {
