...
export class CrossReferenceAnalyzer {
  ...
  async analyzeSources(
    ...
    const embeddings = await Promise.all(
      sources.map(source => this.modelManager.generateEmbedding(source.content))
    )
  ...
  private async findConnections(
    ...
    maxConnections: number,
    contextWindow: number
  ): Promise<Array<{ topic: string; confidence: number; context: { textA: string; textB: string } }>> {
...
${textB}`
    ...
    const result = await this.modelManager.generateText(
      ...
      [
        {
          role: 'system',
          content: 'You are a research analysis assistant specialized in identifying connections between texts. Provide output in JSON format.'
        },
        {
          role: 'user',
          content: prompt
