...
export type DetectionSource = 'url' | 'cookie' | 'localStorage' | 'navigator' | 'htmlLang';
...
export function useLanguageDetection(options: LanguageDetectionOptions = {}) {
  const mergedOptions: Required<LanguageDetectionOptions> = { ...DEFAULT_OPTIONS, ...options };
  ...
  const changeLanguage = (newLocale: Locale) => {
    ...
    if (pathname) {
      ...
      const redirectPath = newLocale === defaultLocale 
        ? newPath 
    ...
    switch (source) {
      case 'url': {
        ...
        if (match) {
          ...
          if (locales.includes(segment as Locale)) {
            ...
            if (options.debug) {
              console.log(`[useLanguageDetection] Detected from URL: ${detectedLocale}`);
            }
          }
        }
        break;
      ...
      case 'cookie': {
        ...
        if (cookieValue && locales.includes(cookieValue as Locale)) {
          ...
          if (options.debug) {
            console.log(`[useLanguageDetection] Detected from cookie: ${detectedLocale}`);
          }
        }
        break;
      }
      ...
      case 'localStorage': {
        ...
        try {
          ...
          if (storedLocale && locales.includes(storedLocale as Locale)) {
            ...
            if (options.debug) {
              console.log(`[useLanguageDetection] Detected from localStorage: ${detectedLocale}`);
            }
          }
        } catch (error) {
      ...
      case 'navigator': {
        ...
        if (typeof navigator !== 'undefined') {
          ...
          if (detectedLocale && options.debug) {
            console.log(`[useLanguageDetection] Detected from navigator: ${detectedLocale}`);
          }
        }
        break;
      }
      ...
      case 'htmlLang': {
        ...
        if (htmlLang && locales.includes(htmlLang as Locale)) {
          ...
          if (options.debug) {
            console.log(`[useLanguageDetection] Detected from HTML lang: ${detectedLocale}`);
          }
        }
        break;
      }
