...
import { userProfileRepository } from "@/_infrastructure/repositories";
import {
...
export class UserApplicationService {
  ...
  async createOrUpdateUser(
    ...
    const dbSettings: Partial<UserProfile> = {
      ...
      ...(validatedSettings.theme && {
        settings_theme: validatedSettings.theme,
      }),
      ...(validatedSettings.language && {
        settings_language: validatedSettings.language,
      }),
  ...
  async updateAvatar(userId: string, avatarUrl: string): Promise<User> {
    ...
    // Map to database structure and save
    const savedProfile = await this.userRepository.updateAvatar(
      userId,
      sanitizedUrl,
    );
  ...
  async applyDefaultSettings(userId: string): Promise<User> {
    ...
    const userWithDefaults =
      this.userService.applyDefaultSettings(existingUser);
