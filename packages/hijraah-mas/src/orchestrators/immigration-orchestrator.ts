...
export class ImmigrationOrchestrator {
  ...
  private async analyzeDocuments(documents: CaseData["documents"]) {
    const { object: analysis } = await generateObject({
      ...
      system:
        "You are an expert immigration document analyst. Analyze documents for completeness, authenticity, and quality.",
  ...
  private async checkPolicyCompliance(
    ...
    const { object: compliance } = await generateObject({
      ...
      system:
        "You are an immigration policy compliance expert. Check applications against current immigration policies.",
  ...
  private async assessRisk(caseData: CaseData, context: Record<string, any>) {
    const { object: riskAssessment } = await generateObject({
      ...
      system:
        "You are a risk assessment specialist for immigration cases. Identify and evaluate risk factors.",
  ...
  private async checkEligibility(
    ...
    const { object: eligibility } = await generateObject({
      ...
      system:
        "You are an immigration eligibility specialist. Determine if applicants meet requirements.",
  ...
  private async synthesizeResults(
    ...
    const { text: summary } = await generateText({
      model: openai("gpt-4o"),
      system:
        "You are an immigration case manager. Synthesize analysis results into actionable recommendations.",
  ...
  private sortTasksByPriority(
    tasks: ProcessingPlan["tasks"]
  ): ProcessingPlan["tasks"] {
