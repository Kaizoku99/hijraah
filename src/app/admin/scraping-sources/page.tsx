...
export default function ScrapingSourcesPage() {
    ...
    const [currentSource, setCurrentSource] = useState<ScrapingSource | null>(null);
    ...
    const onSubmit = async (values: SourceFormValues) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to ${currentSource ? 'update' : 'create'} source`);
            }
    ...
    const handleValidateSource = async (values: ValidationFormValues) => {
        ...
        try {
            const response = await fetch(`/api/admin/scraping-sources/${currentSource.id}/validate`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input placeholder="Canadian Immigration Official Site" {...field} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                            ...
                            {filteredSources.map((source) => (
                                <Card key={source.id} className="overflow-hidden">
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    ...
                                                    <DropdownMenuItem onClick={() => handleManualScrape(source.id)}>
                                                    ...
                                                    <AlertDialog>
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    This will permanently delete the scraping source "{source.name}"
                                    ...
                                    <CardContent className="pb-2">
                                        ...
                                        {source.description && (
                                            <p className="text-sm mt-2 line-clamp-2">{source.description}</p>
                                        )}
                                    </CardContent>
                                    <CardFooter className="flex justify-between pt-2">
                                        ...
                                        <div className="flex items-center">
                                            ...
                                            >
                                                ...
                                                <span className="text-sm font-medium">{source.trust_score}</span>
                                            </div>
                                        </div>
                                    </CardFooter>
                                </Card>
                    ...
                    {currentSource && (
                        <>
                            <div className="py-4">
                                ...
                                <p className="text-sm text-muted-foreground">{currentSource.url}</p>
                                <div className="flex items-center mt-2">
                                    <div className="flex items-center gap-1 rounded-full px-2 py-1 bg-muted">
                                        <Shield className="h-4 w-4" />
                                        <span className="text-sm font-medium">Current score: {currentSource.trust_score}</span>
                                    </div>
                                </div>
                            </div>
                            <Form {...validationForm}>
                                <form onSubmit={validationForm.handleSubmit(handleValidateSource)} className="space-y-4">
                                    <FormField
