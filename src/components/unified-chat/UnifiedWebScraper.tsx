...
import { Search, Link as LinkIcon, Loader2, X, AlertCircle } from 'lucide-react'
...
Would you like me to analyze this information further?`
    ...
    return (
        <div className={`fixed inset-y-0 right-0 w-[400px] border-l bg-background p-4 overflow-auto ${className}`}>
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                    <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setActiveView(activeView === 'form' ? 'results' : 'form')}
                    >
                    ...
                    <Button
                        ...
                        onClick={() => document.dispatchEvent(new CustomEvent('toggle-document-processor'))}
                    >
                        <X className="h-4 w-4" />
            ...
            {activeView === 'form' ? (
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-4">
                            ...
                            <div className="pt-2 border-t">
                                <Label htmlFor="bulk-urls" className="block mb-2">Bulk URLs (one per line)</Label>
                                <Textarea
                                    id="bulk-urls"
                ...
                <div className="space-y-4">
                    <h3 className="font-medium">Scraping Results ({scrapingResults.length})</h3>
                        ...
                        <ScrollArea className="h-[calc(100vh-120px)]">
                            <div className="space-y-4 pr-4">
                                {scrapingResults.map((result, index) => (
                                    <Card key={`${result.url}-${index}`} className="overflow-hidden">
                                            ...
                                            <div className="flex flex-wrap gap-1 mt-1">
                                                ...
                                                {result.metadata?.immigrationData && (
                                                    <Badge variant="secondary" className="text-xs">
                                                        Score: {result.metadata.immigrationData.credibilityScore}%
                                                    </Badge>
                                                )}
                                            </div>
                                        </CardHeader>
                                        <CardContent className="text-sm">
                                            {result.metadata?.immigrationData ? (
                                                <div className="space-y-2">
                                                    <div>
                                                        <span className="font-medium">Type:</span> {result.metadata.immigrationData.documentType} ({result.metadata.immigrationData.sourceType})
                                                    </div>
                                                    {result.metadata.immigrationData.countries && (
                                                        <div>
                                                            <span className="font-medium">Countries:</span> {result.metadata.immigrationData.countries.join(', ')}
                                                        </div>
                                                    )}
                                                    {result.metadata.immigrationData.visaTypes && (
                                                        <div>
                                                            <span className="font-medium">Visa Types:</span> {result.metadata.immigrationData.visaTypes.join(', ')}
                                                        </div>
                                                    )}
                                                    {result.metadata.immigrationData.requirements && (
                                                        <div>
                                                            ...
                                                            <ul className="list-disc pl-5">
                                                                {result.metadata.immigrationData.requirements.map((req, i) => (
                                                                    <li key={i}>{req}</li>
                                                                ))}
                                                            </ul>
                                                        </div>
                                                ...
                                                <UnifiedMarkdown content={result.content.substring(0, 200) + '...'} />
                                            )}
                                            <Button
                                                variant="link"
