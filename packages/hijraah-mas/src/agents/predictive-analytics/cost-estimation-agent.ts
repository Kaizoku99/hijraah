...
export class CostEstimationAgent {
  ...
  async estimateCosts(
    ...
    const governmentFeesTool = tool({
      ...
      execute: async ({ visaType, country, familyMembers, expeditedProcessing }) => {
        const baseFees = this.getGovernmentFees(visaType, country)
        const expeditedFee = expeditedProcessing ? baseFees.application * 0.5 : 0
    ...
    const legalFeesTool = tool({
      ...
      parameters: z.object({
        visaType: z.string(),
        caseComplexity: z.enum(['simple', 'moderate', 'complex', 'very_complex']),
        includeLegalFees: z.boolean(),
        country: z.string()
      }),
      execute: async ({ visaType, caseComplexity, includeLegalFees, country }) => {
        if (!includeLegalFees) return []

        const baseLegalFee = this.getLegalFees(visaType, caseComplexity, country)
    ...
    const documentFeesTool = tool({
      ...
      execute: async ({ userProfile, visaType, country }) => {
        ...
        if (userProfile.education?.level && ['bachelor', 'master', 'phd'].includes(userProfile.education.level)) {
          costs.push({
            category: 'document_fees',
    ...
    const medicalExamTool = tool({
      ...
      execute: async ({ visaType, country, age, familyMembers }) => {
        ...
        if (['permanent_residence', 'family_visa', 'work_visa'].includes(visaType)) {
    ...
    const travelCostsTool = tool({
      ...
      execute: async ({ currentCountry, targetCountry, visaType, includeOptionalServices }) => {
        const costs = []
...
Analyze how these changes affect the cost estimation and provide updated totals.`
  ...
  private getGovernmentFees(visaType: string, country: string) {
    ...
    const feeStructures: Record<string, { application: number; biometric: number; dependent: number }> = {
      ...
      'permanent_residence': { application: 1140, biometric: 85, dependent: 1140 },
      'citizenship': { application: 725, biometric: 85, dependent: 0 }
    }
    
    return feeStructures[visaType] || { application: 400, biometric: 85, dependent: 400 }
  }

  private getLegalFees(visaType: string, complexity: string, country: string): number {
