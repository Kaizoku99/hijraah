...
export class SearchEngine {
  ...
  constructor(config: SearchEngineConfig) {
    ...
    this.modelManager = config.modelManager || new AIModelManager({
  ...
  async search(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    ...
    try {
      ...
      // Vector search
      const { data: vectorResults, error } = await this.supabase.rpc('match_documents', {
  ...
  private transformFirecrawlResults(results: FirecrawlSearchResult[]): SearchResult[] {
    return results.map(r => ({
      url: r.url,
      title: r.title,
