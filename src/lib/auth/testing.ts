...
/**
 * Create a mock extended user
 */
export function createMockExtendedUser(overrides: Partial<ExtendedUser> = {}): ExtendedUser {
  const user = createMockUser(overrides);
  
  return {
    ...user,
    fullName: user.user_metadata?.full_name || 'Mock User',
    avatarUrl: user.user_metadata?.avatar_url || 'https://example.com/avatar.png',
 ...
 */
export function createMockAuthProvider(options: {
 ...
 */
export function setupMockMiddlewareContext(options: {
  user?: ExtendedUser | null;
  session?: Session | null;
  isAuthenticated?: boolean;
} = {}) {
 ...
 */
export function mockUseAuth(options: {
  user?: ExtendedUser | null;
  session?: Session | null;
  isLoading?: boolean;
  isAuthenticated?: boolean;
 ...
 */
export function createServerActionContext(options: {
  user?: User | null;
  session?: Session | null;
  error?: Error | null;
} = {}) {
  ...
  return {
    user,
    session,
    mockSupabase: createMockSupabaseClient({ user, session, error: options.error }),
 ...
 */
export function setupAuthComponentTest(options: {
  user?: ExtendedUser | null;
  session?: Session | null;
  isLoading?: boolean;
  isAuthenticated?: boolean;
