...
import { z } from "zod";
import { ProcessedCaseData, ProcessedCaseDataSchema } from "../types";
...
export class CaseDecisionTeam {
  ...
  private async makeFinalDecision(
    ...
    const finalDecisionCall = toolCalls.find(
      (call: any) => call.toolName === "finalDecision"
    );
    if (!finalDecisionCall) {
      throw new Error("No final decision was made");
    }

    return DecisionSchema.parse(finalDecisionCall.input);
  }
  ...
  async generateDecisionDocument(
    ...
    const generateDocument = withAgentErrorHandling(async () => {
      const { object: document } = await generateObject({
        ...
        system:
          "You are a legal document specialist. Generate formal immigration decision documents.",
  ...
  async getProcessingStatistics(
    ...
    const getStats = withAgentErrorHandling(async () => {
      const { object: stats } = await generateObject({
        ...
        system:
          "You are a case processing statistics analyst. Analyze decision patterns and trends.",
