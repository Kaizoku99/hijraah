 ...
 */
export async function getUserPersonalizationProfile(
  userId: string,
 ...
 */
async function createDefaultPersonalizationProfile(
  userId: string,
...
export async function updateUserInterests(
  ...
  if (existingInterestIndex >= 0) {
    ...
    profile.interests[existingInterestIndex].lastUpdated =
...
export async function getRecentChatHistory(
  ...
  const allMessages = (data || []).flatMap(
    (record: any) => record.messages || [],
  );
 ...
 */
export async function generatePersonalizationContext(
  userId: string,
): Promise<string> {
  if (!userId) return "";
