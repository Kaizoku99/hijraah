...
export { storeDocument, getDocumentMetadata, updateDocumentStatus, getDocumentOCR, processBatch } from './document/document-store';
export { checkRateLimit, createRateLimitMiddleware, getUserTier, canProcessBatch } from './rate-limit';
...
export async function getBatchStatus(batchId: string): Promise<{
  ...
  failed: number;
  documentIds: string[];
  results: Array<{ documentId: string; status: string; progress: number; error?: string }>;
}> {
 ...
 */
export async function checkUploadQuota(userId: string, fileSize: number): Promise<{
  allowed: boolean;
  reason?: string;
  quota: {
    total: number;
