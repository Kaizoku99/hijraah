...
export class OCRProcessingAgent {
  ...
  async processOCR(
    ...
    const processOCR = withAgentErrorHandling(async () => {
      ...
      const messageContent = await this.prepareOCRMessageContent(
        documentInput,
        extractionOptions
      );
  ...
  async processOCRWithValidation(
    ...
    const processWithValidation = withAgentErrorHandling(async () => {
      ...
      const primaryResult = await this.processOCR(
        documentInput,
        extractionOptions
      );
      ...
      if (extractionOptions.useMultiModelValidation) {
        const validationResult = await this.validateOCRWithOpenAI(
          documentInput,
          primaryResult
        );
...
- Any handwritten annotations`;
    ...
    switch (documentInput.type) {
        ...
        if (typeof documentInput.source === "string") {
          if (documentInput.source.startsWith("data:")) {
            return [
              ...
              { type: "image" as const, image: documentInput.source },
            ];
          } else {
            return [
              { type: "text" as const, text: basePrompt },
              { type: "image" as const, image: new URL(documentInput.source) },
            ];
          }
        }
        break;
      ...
      case "file_buffer":
        if (
          documentInput.source instanceof Buffer ||
          documentInput.source instanceof ArrayBuffer
        ) {
          const buffer =
            documentInput.source instanceof Buffer
              ? documentInput.source
              : Buffer.from(documentInput.source as ArrayBuffer);
          ...
          return [
            { type: "text" as const, text: basePrompt },
            {
              ...
              data: buffer,
              mediaType:
                documentInput.metadata?.mimeType || "application/octet-stream",
            },
          ];
        }
        break;
      ...
      default:
        throw new Error(
          `Unsupported document type for OCR: ${documentInput.type}`
        );
    }
  ...
  private async validateOCRWithOpenAI(
    ...
    const messageContent = await this.prepareOCRMessageContent(documentInput, {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are validating OCR results. Extract text and compare with the provided results for accuracy.",
        },
        {
          role: "user",
          content: messageContent,
  ...
  private mergeOCRResults(
    ...
    const adjustedConfidence =
      primaryResult.extractedText.confidence * (0.5 + textSimilarity * 0.5);
  ...
  getQualityMetrics(result: OCRProcessingResult): Record<string, number> {
    return {
      ...
      structured_fields_count: Object.keys(result.extractedText.structuredData)
        .length,
