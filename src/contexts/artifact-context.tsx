...
interface ArtifactContextType {
    ...
    isLoading: boolean;
    error: string | null;
    createArtifact: (title: string, type: ArtifactType, content: any, chatId?: string) => Promise<Artifact>;
    updateArtifact: (id: string, updates: Partial<Artifact>) => Promise<void>;
    deleteArtifact: (id: string) => Promise<void>;
    getArtifact: (id: string) => Promise<void>;
    addArtifactMessage: (artifactId: string, message: string, role: 'user' | 'assistant' | 'system') => Promise<void>;
    updateArtifactVisibility: (id: string, visibility: ArtifactVisibility) => Promise<void>;
}
...
export function ArtifactProvider({ children }: { children: React.ReactNode }) {
        ...
        const subscription = supabase
            ...
            .on('postgres_changes', {
                ...
                if (payload.eventType === 'INSERT') {
                    ...
                    setArtifacts(prev =>
                        prev.map(artifact =>
                            artifact.id === payload.new.id ? payload.new as Artifact : artifact
                        )
                    );
    ...
    const updateArtifactVisibility = async (id: string, visibility: ArtifactVisibility) => {
        try {
            setIsLoading(true);
            setError(null);
