...
export function useDeepResearch() {
  ...
  const pollForUpdates = useCallback(async (sessionId: string) => {
    try {
      const response = await fetch(`/api/deep-research?sessionId=${sessionId}`);
      ...
      if (data.success) {
        ...
        if (session && session.metadata) {
          ...
          if (typeof current_depth === 'number' && typeof max_depth === 'number') {
        ...
        if (findings && Array.isArray(findings)) {
          findings.forEach(finding => {
            addActivity({
              type: finding.type === 'error' ? 'error' : 
                   finding.type === 'search_queries' ? 'search' :
                   finding.type === 'content_extraction' ? 'extract' :
                   finding.type === 'synthesis' ? 'synthesis' :
                   finding.type === 'final_answer' ? 'reasoning' : 'analyze',
