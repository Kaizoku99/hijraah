...
app.use('*', async (c, next) => {
  ...
  try {
    const { success, limit, remaining, reset } = await globalRateLimiter.limit(identifier);
    ...
    if (!success) {
      logger.warn('Global rate limit exceeded', { ip, path: c.req.path, method: c.req.method });
      return c.json(
        { 
    ...
    logger.error('Error during global rate limiting', errorToLog, { ip, path: c.req.path, method: c.req.method });
...
app.onError((err: Error | HTTPException, c: Context) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    return c.json({
      success: false,
      error: status === 500 ? 'An unexpected error occurred' : errorToLog.message,
      requestId
    }, status);
  } else {
    return c.json({
...
async function startServer() {
  try {
    ...
    if (!isHealthy) {
      logger.error('Health check failed during startup, unable to start server');
      process.exit(1);
    }
