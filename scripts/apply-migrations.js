...
async function applyMigration(filePath) {
  try {
    ...
    if (error) {
      console.error(
        `Error applying migration ${path.basename(filePath)}:`,
        error,
      );
      return false;
    }
    ...
    console.log(
      `✅ Successfully applied migration: ${path.basename(filePath)}`,
    );
    return true;
  } catch (err) {
    console.error(
      `Error reading or applying migration ${path.basename(filePath)}:`,
      err,
    );
    return false;
  }
...
async function main() {
  ...
  const migrationPath = path.join(
    __dirname,
    "../src/lib/db/migrations/create_admin_users_table.sql",
  );
