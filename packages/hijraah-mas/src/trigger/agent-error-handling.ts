...
export class AgentErrorHandler {
  ...
  constructor(retryStrategy: RetryStrategyConfig = DEFAULT_RETRY_STRATEGY, logger?: any) {
  ...
  async handleError(
    ...
    if (!this.isRetryableError(errorType) || 
      ...
      return await this.applyFallbackStrategy(error, context, originalExecution)
    }
    ...
    try {
      ...
      return await this.handleError(retryError as Error, updatedContext, originalExecution)
    }
  }
  ...
  private async applyFallbackStrategy(
    ...
    const strategy = context.recovery?.strategy || this.retryStrategy.fallbackStrategy
  ...
  private async executeFallbackAgent(context: AgentErrorContext): Promise<any> {
    const fallbackAgentType = context.recovery?.fallbackAgent || 'immigration_orchestrator'
   ...
   */
  private async executeGracefulDegradation(context: AgentErrorContext): Promise<any> {
  ...
  private classifyError(error: Error): AgentErrorType {
    ...
    if (message.includes('model') || message.includes('ai') || message.includes('openai')) {
    ...
    if (message.includes('dependency') || message.includes('service unavailable')) {
...
export function withAgentErrorHandling<T>(
  ...
  return agentExecution().catch(async (error: Error) => {
    ...
    return await errorHandler.handleError(error, updatedContext, agentExecution)
  })
}
 ...
 */
export function createAgentRetryStrategy(agentType: string): RetryStrategyConfig {
...
export class AgentErrorMetrics {
  ...
  recordError(agentType: string, errorType: AgentErrorType, recoveryStrategy?: ErrorRecoveryStrategy) {
    ...
    if (recoveryStrategy) {
      const currentStrategyCount = this.recoveryStrategies.get(recoveryStrategy) || 0
