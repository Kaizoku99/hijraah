...
import { Artifact as ArtifactType, ArtifactVisibility, DocumentContent, CodeContent } from '@/types/artifact';
import { useArtifact } from '@/contexts/artifact-context';
import { BadgeCheck, Code, FileText, Grid3X3, Image, Lock, Share2, Trash2, X } from 'lucide-react';
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
export function Artifact({ id, viewOnly = false, onClose, className }: ArtifactProps) {
    const { currentArtifact, artifactMessages, getArtifact, updateArtifact, deleteArtifact, updateArtifactVisibility, isLoading } = useArtifact();
    const [activeTab, setActiveTab] = useState('document');
    const [isEditing, setIsEditing] = useState(false);
    const [editedTitle, setEditedTitle] = useState('');
    const [documentContent, setDocumentContent] = useState<DocumentContent>({ text: '' });
    const [codeContent, setCodeContent] = useState<CodeContent>({ code: '', language: '' });
    ...
    const handleSaveChanges = async () => {
        ...
        const updatedContent =
            currentArtifact.type === 'document' ? documentContent :
                currentArtifact.type === 'code' ? codeContent :
    ...
    if (isLoading || !currentArtifact) {
        return (
            <Card className={cn("w-full h-full min-h-[300px] flex items-center justify-center", className)}>
                <div className="animate-pulse text-muted-foreground">Loading artifact...</div>
            </Card>
        );
    }
        ...
        <Card className={cn("w-full h-full flex flex-col", className)}>
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-8 w-8">
                                        {getVisibilityIcon(currentArtifact.visibility as ArtifactVisibility)}
                                    </Button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="end">
                                    ...
                                    <DropdownMenuItem onClick={() => handleUpdateVisibility('team')}>
            ...
            <CardContent className="flex-1 p-0 overflow-hidden">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
                    <TabsList className="mx-4">
                        <TabsTrigger value="document" onClick={() => setActiveTab('document')}>Document</TabsTrigger>
                                ...
                                <div className="min-h-[300px] font-mono">
                                    <textarea
                                        ...
                                        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                                            setDocumentContent({ ...documentContent, text: e.target.value })}
                                        className="w-full h-full min-h-[300px] p-2 border rounded-md"
                                    />
                                </div>
                            ) : (
                                <div className="prose dark:prose-invert max-w-none">
                                    <SimpleMarkdownViewer content={currentArtifact.content?.text || ''} />
                                </div>
                            )
                        )}
                                    ...
                                    <div className="flex space-x-2">
                                        <Input
                                            ...
                                            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                                setCodeContent({ ...codeContent, fileName: e.target.value })}
                                        ...
                                        <Input
                                            ...
                                            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                                setCodeContent({ ...codeContent, language: e.target.value })}
            ...
            <CardFooter className="text-xs text-muted-foreground pt-2 pb-2">
                <div className="flex items-center space-x-1">
                    <span>Created: {new Date(currentArtifact.created_at).toLocaleString()}</span>
                    <span>•</span>
                    <span>Updated: {new Date(currentArtifact.updated_at).toLocaleString()}</span>
                </div>
            </CardFooter>
        </Card>
    );
