  ...
  for (const stepBatch of executionQueue) {
    ...
    const batchPromises = stepBatch.map(async (step) => {
      try {
        ...
        if (
          step.condition &&
          !evaluateCondition(step.condition, stepResults, context)
        ) {
...
function buildDependencyGraph(
  steps: AgentWorkflowConfig["steps"]
): Map<string, string[]> {
...
): Array<Array<AgentWorkflowConfig["steps"][0]>> {
...
export const immigrationCaseProcessingWorkflow: AgentWorkflowConfig = {
  ...
  description:
    "Comprehensive workflow for processing immigration cases from document analysis to final decision",
  steps: [
    {
    ...
    },
    {
      ...
      condition:
        'results["policy-compliance"].result.overallCompliance !== "non_compliant"',
...
export const policyChangeImpactWorkflow: AgentWorkflowConfig = {
  ...
  description:
    "Analyze policy changes and assess their impact across jurisdictions",
  steps: [
    {
    ...
    },
    {
      ...
      input: {
        policyChanges:
          "${dependencies.policy-monitoring.result.monitoringResults}",
    ...
    },
    {
      ...
      input: {
        ...
        context: {
          impact: "${dependencies.impact-assessment.result}",
          crossJurisdiction:
            "${dependencies.cross-jurisdiction-analysis.result}",
        },
      },
      dependencies: [
        "policy-monitoring",
