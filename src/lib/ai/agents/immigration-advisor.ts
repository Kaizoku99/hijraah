    ...
    try {
      ...
      if (delegationResult.shouldDelegate) {
        ...
        const delegationMessage: ChatMessage = {
          id: Date.now().toString(),
          role: 'assistant',
          content: `I'm connecting you with our ${delegationResult.agentType.replace('_', ' ')} specialist who can better help with this specific question.`,
          sessionId: this.id,
          createdAt: new Date().toISOString(),
        };
...
${data.map((interaction: any) => `- ${interaction.interaction_type} (${interaction.category || 'general'})`).join('\n')}
        `;
      }
    }
