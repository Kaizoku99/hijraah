...
import { Loader2, Globe, LinkIcon, Eye, Brain, BookOpen, DatabaseIcon } from 'lucide-react';
    ...
    const onSubmit = async (data: WebScraperFormValues) => {
        ...
        try {
            ...
            if (data.extractSelectors) {
                try {
                    extractSelectors = JSON.parse(data.extractSelectors) as Record<string, string>;
                } catch (error) {
                ...
                <CardContent>
                    <Tabs defaultValue={activeTab}>
                        <TabsList className="mb-4">
                            ...
                            <TabsTrigger value="preview" disabled={!scrapedContent} onClick={() => setActiveTab('preview')}>
                                        ...
                                        <AccordionContent>
                                            <div className="space-y-4 pt-2">
                                                <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>Wait for selectors (comma separated)</FormLabel>
                                                            <FormControl>
                                                                <Input placeholder="#main-content, .article-body" {...field} />
                                                            </FormControl>
                                                            <p className="text-sm text-gray-500">
                                                ...
                                                <FormField
                                                    ...
                                                    render={({ field }) => (
                                                        <FormItem>
                                                            <FormLabel>Extract selectors (JSON or key:value format)</FormLabel>
                                                            <FormControl>
                                                                <Textarea
                            ...
                            {scrapedContent && (
                                <div className="space-y-6">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {scrapedContent.immigrationData.documentType && (
                                                        <div>
                                                            ...
                                                            <p>{scrapedContent.immigrationData.documentType}</p>
                                                        </div>
                                                    )}
                                                    ...
                                                    {scrapedContent.immigrationData.credibilityScore && (
                                                        <div>
                                                            ...
                                                            <p>{scrapedContent.immigrationData.credibilityScore}/100</p>
                                                        </div>
                                                    )}

                                                    {scrapedContent.immigrationData.countries && scrapedContent.immigrationData.countries.length > 0 && (
                                                        <div>
                                                            <h4 className="font-medium">Countries</h4>
                                                            <p>{scrapedContent.immigrationData.countries.join(', ')}</p>
                                                        </div>
                                                    )}

                                                    {scrapedContent.immigrationData.visaTypes && scrapedContent.immigrationData.visaTypes.length > 0 && (
                                                        <div className="col-span-2">
                                                            <h4 className="font-medium">Visa Types</h4>
                                                            <p>{scrapedContent.immigrationData.visaTypes.join(', ')}</p>
                                                        </div>
                                                    )}

                                                    {scrapedContent.immigrationData.requirements && scrapedContent.immigrationData.requirements.length > 0 && (
                                                        <div className="col-span-2">
                                                            <h4 className="font-medium">Requirements</h4>
                                                            <ul className="list-disc pl-5">
                                                                {scrapedContent.immigrationData.requirements.map((req: string, i: number) => (
                                                                    <li key={i}>{req}</li>
                                                                ))}
                                                            </ul>
                                                        </div>
                                                    ...
                                                    {scrapedContent.immigrationData.keyPoints && scrapedContent.immigrationData.keyPoints.length > 0 && (
                                                        <div className="col-span-2">
                                                            <h4 className="font-medium">Key Points</h4>
                                                            <ul className="list-disc pl-5">
                                                                {scrapedContent.immigrationData.keyPoints.map((point: string, i: number) => (
                                                                    <li key={i}>{point}</li>
                                                                ))}
                                                            </ul>
                                                        </div>
                                    ...
                                    <div className="flex justify-between">
                                        <Button variant="outline" onClick={() => setActiveTab('url')}>
                                            Back to URL Input
                                        </Button>
