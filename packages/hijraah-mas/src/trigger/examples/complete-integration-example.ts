...
export async function executeImmigrationCaseWorkflow() {
  ...
  try {
    ...
    console.log('Immigration case workflow completed:', {
      ...
      success: result.success,
      completedSteps: result.result.completedSteps,
      totalSteps: result.result.totalSteps,
      duration: result.execution.duration
    })
...
export async function executePolicyChangeAnalysis() {
  ...
  try {
    ...
    console.log('Policy change analysis workflow completed:', {
      workflowId: result.workflowId,
      success: result.success,
      changesDetected: result.result.finalResult?.workflowSummary?.keyResults?.length || 0,
      duration: result.execution.duration
    })
...
export async function demonstrateErrorHandling() {
  ...
  const riskyAgentExecution = async () => {
    ...
    const failureTypes = ['timeout', 'rate_limit', 'model_error', 'validation_error']
    const randomFailure = failureTypes[Math.floor(Math.random() * failureTypes.length)]
    
    if (Math.random() < 0.7) { // 70% chance of failure for demonstration
...
export async function monitorAgentPerformance() {
  ...
  const tasks = [
    { agentType: 'policy_monitoring_agent', input: { sources: ['https://example.com'] } },
  ...
  for (const [index, taskConfig] of tasks.entries()) {
    ...
    try {
      ...
      performanceMetrics.totalTokenUsage += result.execution.totalTokenUsage?.totalTokens || 0
      
      const responseTime = Date.now() - taskStartTime
      performanceMetrics.averageResponseTime = 
        (performanceMetrics.averageResponseTime * (performanceMetrics.tasksExecuted - 1) + responseTime) / 
        performanceMetrics.tasksExecuted
  ...
  console.log('Performance monitoring results:', {
    ...performanceMetrics,
    totalDuration,
    successRate: (performanceMetrics.successfulTasks / performanceMetrics.tasksExecuted) * 100,
    averageTokensPerTask: performanceMetrics.totalTokenUsage / performanceMetrics.tasksExecuted
  })
