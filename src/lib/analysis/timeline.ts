...
export class TimelineAnalyzer {
  ...
  private getSegmentEndDate(date: Date, granularity: TimelineOptions['granularity']): Date {
  ...
  private async generateSegmentSummary(events: TimelineEvent[]): Promise<string> {
    const prompt = `Summarize the following events chronologically, highlighting key developments and connections:

${events.map(e => `Date: ${e.date.toISOString()}
Title: ${e.title}
...
`).join('\n---\n')}`
    ...
    const summary = await this.modelManager.generateText(
      ...
      [
        {
          role: 'system',
          content: 'You are a timeline analysis assistant. Create a concise summary that captures the key developments and their significance.'
        },
        {
          role: 'user',
          content: prompt
  ...
  private async identifyTrends(events: TimelineEvent[]): Promise<Array<{
    name: string
    description: string
    strength: number
  }>> {
...
3. Strength (0-1) based on prevalence and impact`
    ...
    const result = await this.modelManager.generateText(
      ...
      [
        {
          role: 'system',
          content: 'You are a trend analysis assistant. Identify and explain significant patterns in chronological data.'
        },
        {
          role: 'user',
          content: prompt
