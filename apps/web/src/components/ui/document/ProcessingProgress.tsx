...
export function ProcessingProgress({
  ...
  }, [documentId, pollInterval, processingState.status, fetchProcessingStatus]);

  // Format the time elapsed
  const formatTimeElapsed = () => {
    const elapsed =
      new Date().getTime() - processingState.lastUpdated.getTime();
      ...
      case "failed":
        return (
          <div className="flex items-center space-x-2 text-red-600">
            ...
            <span>
              Processing failed: {processingState.error || "Unknown error"}
            </span>
          </div>
        );
    ...
    <Card className="mb-4">
      <CardContent className="py-4">
        <div className="space-y-3">
          ...
          <div className="text-sm">{renderStatusContent()}</div>
