...
export const requirementCheckTool = createImmigrationTool(
  ...
  async ({ applicationId, requirements, applicationData, strictMode }) => {
    ...
    const metCount = mockRequirementCheck.results.filter(r => r.met).length
    ...
    mockRequirementCheck.summary.complianceScore = Math.floor((metCount / requirements.length) * 100)

    // Add gaps and recommendations for unmet requirements
    mockRequirementCheck.results.forEach(result => {
      if (!result.met) {
        result.gaps.push(`Missing or insufficient evidence for: ${result.requirement}`)
        result.recommendations.push(`Provide additional documentation for: ${result.requirement}`)
    ...
    if (mockRequirementCheck.criticalGaps.length > 0) {
      mockRequirementCheck.nextSteps.push('Address critical requirement gaps immediately')
    }
    if (unmetCount > 0) {
      mockRequirementCheck.nextSteps.push('Submit additional documentation for unmet requirements')
    }
    if (mockRequirementCheck.summary.overallCompliance) {
      mockRequirementCheck.nextSteps.push('Proceed to next stage of application processing')
    }
